            ...
            <div ref={chatScrollRef} className="flex-1 overflow-y-auto overflow-x-hidden p-4 custom-scrollbar bg-slate-50 dark:bg-slate-950 space-y-4 min-h-0">
              ...
              {messages.map((m, idx) => (
                <div key={idx} className={`flex w-full ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`p-3 rounded-2xl shadow-sm ${m.role === 'user' ? 'max-w-[80%] bg-indigo-600 text-white rounded-tr-sm' : 'max-w-[95%] bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-100 dark:border-slate-700 rounded-tl-sm'}`}>
                    {m.role === 'model' && (
              ...
              {isTyping && (
                <div className="flex justify-start w-full">
                  <div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 p-3 rounded-2xl rounded-tl-sm shadow-sm flex flex-col gap-2 max-w-[95%]">
                    <div className="flex items-center gap-2">
