    ...
    const renderItems = () => {
        if (mode === 'section') {
            ...
            return safeSections.map((sec, sIdx) => {
                ...
                return (
                    <div key={sec.id || sIdx} className="mb-3">
                        {/* Section Header Indicator */}
                        <div className="flex items-center gap-1 px-1.5 py-1 mb-1 bg-indigo-50/90 rounded border border-indigo-200/80 text-[9px] font-bold text-indigo-700 truncate shadow-2xs">
                            <Layers className="w-2.5 h-2.5 text-indigo-600 shrink-0" />
                            <span className="truncate">{sec.name || `Section ${sIdx + 1}`}</span>
                        </div>
                            ...
                            {secQuestions.map((q, qIdx) => {
                                ...
                                return (
                                    <div
                                        ...
                                        className={`group/qitem relative cursor-pointer p-1.5 rounded-lg border transition-all duration-150 ${
                                            isCurrentActive
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center justify-between gap-1 mb-1">
                                            <div className="flex items-center gap-1">
                                                ...
                                                {isPassage && (
                                                    <span className={`text-[7px] font-bold px-1 rounded uppercase ${isCurrentActive ? 'bg-indigo-700 text-indigo-100' : 'bg-indigo-100 text-indigo-700'}`}>
                                                        PAS
                                                    </span>
                                                )}
                                            </div>
                                            <span className={`text-[8px] font-semibold ${isCurrentActive ? 'text-indigo-100' : 'text-slate-400'}`}>
                                                {q.marks || 1}M
                                            </span>
                                        </div>
                                        ...
                                        <div className="space-y-0.5">
                                            ...
                                            <div className="pt-0.5 grid grid-cols-2 gap-0.5">
    ...
    return (
        <div className="fixed right-3 top-24 bottom-6 z-40 flex items-start pointer-events-none select-none">
            ...
            <button
                ...
                className="pointer-events-auto bg-white text-slate-700 hover:text-indigo-600 border border-slate-200 hover:border-slate-300 shadow-md rounded-l-lg p-1.5 transition-all cursor-pointer backdrop-blur-md self-center"
            ...
            {!collapsed && (
                <div 
                    ref={minimapRef}
                    className="pointer-events-auto w-38 h-full bg-white/95 border border-slate-200/90 rounded-r-xl shadow-xl backdrop-blur-md flex flex-col overflow-hidden relative transition-all duration-300"
                >
                    {/* Header Bar */}
                    <div className="px-2.5 py-2 bg-slate-50 border-b border-slate-200 flex items-center justify-between shrink-0">
                        ...
                        <span className="text-[9px] font-mono font-bold px-1.5 py-0.5 bg-indigo-50 text-indigo-700 rounded border border-indigo-200/80">
                            {totalQuestions} Qs
                        </span>
                    </div>
                    ...
                    <div 
                        ...
                        className="flex-1 overflow-y-auto p-2 scrollbar-thin scrollbar-thumb-slate-300 relative space-y-1"
                    >
                        {renderItems()}
                    </div>
                </div>
