...
-> Give full output in one **code snippet** only.`;
  ...
  const content = (
    <div className="space-y-8 py-4 max-w-full overflow-x-hidden">
      ...
      {/* Step 3: Copy AI JSON Output */}
      <div className="flex gap-4">
        <div className="flex-none w-8 h-8 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-bold">3</div>
        <div className="space-y-3 flex-1 min-w-0">
          <h3 className="font-semibold text-lg text-blue-700">Copy the JSON Output</h3>
          <p className="text-sm text-muted-foreground">
            Simply copy the structured JSON code generated by Google Gemini, ChatGPT, or Perplexity to your clipboard.
          </p>
          {/* <div className="border border-dashed border-slate-200 dark:border-slate-800 rounded-xl p-4 bg-slate-50/50 dark:bg-slate-900/50">
            <h4 className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-2">Optional: Offline Template fallback</h4>
            <p className="text-xs text-muted-foreground mb-3">If you prefer working with local files, you can download our blank JSON template structure:</p>
            <Button onClick={handleDownload} variant="outline" size="sm" className="gap-2 border-slate-200 bg-white hover:bg-slate-50 dark:bg-slate-950 dark:hover:bg-slate-900 dark:border-slate-800">
      ...
      <div className="flex gap-4">
        ...
        <div className="space-y-3 flex-1 min-w-0">
          <h3 className="font-semibold text-lg text-green-700">Paste & Import Directly</h3>
          {/* <p className="text-sm text-muted-foreground">
            No need to create files or visit other websites. Import directly in two quick steps:
          </p> */}
          <ol className="text-sm text-slate-600 dark:text-slate-400 space-y-2 list-decimal list-inside pl-1">
            <li>On the Test Builder, click the <strong className="text-slate-800 dark:text-slate-200">Import JSON</strong> button and select <strong className="text-indigo-600 dark:text-indigo-400">Paste JSON</strong>.</li>
            <li>Paste your copied text. Use the <strong className="text-slate-800 dark:text-slate-200">Auto Repair</strong> button if there are formatting errors, and click <strong className="text-slate-800 dark:text-slate-200">Import</strong>.</li>
          </ol>
          <p className="text-xs text-muted-foreground italic mt-2 border-t pt-2 border-slate-100 dark:border-slate-800">
            * Note: You can still use the traditional method by selecting <strong>Upload File</strong> to upload any saved <code>.json</code> file.
          </p>
        </div>
      </div>

      {/* Old Step 3 and Step 4 commented out as per instructions */}
      {/* 
          ...
          <CollapsibleContent className="space-y-4">
            <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-lg text-sm space-y-6">
              ...
              <div>
                ...
                <pre className="bg-slate-950 text-slate-50 p-3 rounded text-xs font-mono overflow-x-auto max-w-full">
                  {`{
