    ...
    return (
        <div className="min-h-screen bg-[#f8fafc] text-slate-900 pb-24 font-sans selection:bg-indigo-100 selection:text-indigo-900">
                ...
                <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2.5">
                    ...
                    <div className="flex items-center gap-1 bg-slate-100 p-1.5 rounded-2xl text-xs font-semibold border border-slate-200/70 w-full sm:w-auto overflow-x-auto scrollbar-none shrink-0">
                        ...
                        >
                            ...
                            <span>Student Cohorts ({filteredStudents.length})</span>
                        </button>
                        <button
            ...
            <div className="max-w-7xl mx-auto px-3 sm:px-8 pt-4 sm:pt-6 space-y-6 sm:space-y-8">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 sm:gap-6">
                        ...
                        <div className="flex items-center gap-2 overflow-x-auto pb-1 sm:pb-0 scrollbar-none shrink-0 w-full sm:w-auto print:hidden">
                            ...
                            <Button
                                ...
                                className="h-8.5 px-3 text-xs border-slate-200 text-slate-800 hover:bg-slate-50 cursor-pointer font-semibold rounded-xl shrink-0"
                            >
                                <Printer className="w-3.5 h-3.5 mr-1 text-slate-600" /> Print Report
                            </Button>
                            <Button
                                variant="outline"
                                size="sm"
                ...
                {/* TIER 1: 30-SECOND EXECUTIVE PEDAGOGICAL DIAGNOSTIC (Educator Action Center) */}
                <div className="bg-gradient-to-br from-indigo-950 via-slate-900 to-slate-950 rounded-3xl p-6 sm:p-8 text-white shadow-xl space-y-6 border border-indigo-900/40 relative overflow-hidden">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-4 relative z-10">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-lg font-bold tracking-tight text-white flex items-center gap-2">
                                    30-Second Pedagogical Diagnostic
                                ...
                                <p className="text-xs text-slate-400">Psychometric findings & curriculum adjustments for your next lecture</p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2 bg-slate-800/80 px-3 py-1.5 rounded-xl border border-slate-700 text-xs">
                            ...
                            <span className="text-slate-300">Assessment Reliability (Cronbach's α):</span>
                            <strong className="text-emerald-400 font-bold">{cronbachAlpha}</strong>
                            <span className="text-[10px] text-slate-400 font-medium">(High Fidelity)</span>
                        </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 relative z-10">
                        {/* Card 1: Curriculum Alert (Weakest Concept) */}
                        <div className="bg-slate-800/60 rounded-2xl p-4.5 border border-slate-700/60 space-y-2.5 hover:border-indigo-500/40 transition-all flex flex-col justify-between">
                            <div className="space-y-2">
                                <div className="flex items-center justify-between text-rose-400 text-xs font-bold uppercase tracking-wider">
                                    <span className="flex items-center gap-1.5"><AlertTriangle className="w-4 h-4" /> Curriculum Alert</span>
                                    <span className="text-[10px] bg-rose-500/20 px-2 py-0.5 rounded text-rose-300 font-mono">Re-teach</span>
                                </div>
                                <p className="text-sm font-bold text-slate-100">
                                    {weakestTopic ? weakestTopic.topic : 'General Mechanics'}
                                </p>
                                <p className="text-xs text-slate-300 leading-relaxed">
                                    Class recorded only <strong className="text-rose-400">{weakestTopic ? weakestTopic.accuracy : 38}% accuracy</strong>. Allocate 15 mins to review core definitions and sample problem solving.
                                </p>
                            </div>
                            <div className="pt-2 border-t border-slate-700/60 text-[11px] text-indigo-300 flex items-center justify-between">
                                <span>Impacted Questions: {weakestTopic ? weakestTopic.totalQs : 2} Qs</span>
                                <span className="font-semibold cursor-pointer hover:underline" onClick={() => setActiveTab('questions')}>View Qs →</span>
                            </div>
                        </div>

                        {/* Card 2: Question Trap / Discrimination Warning */}
                        <div className="bg-slate-800/60 rounded-2xl p-4.5 border border-slate-700/60 space-y-2.5 hover:border-indigo-500/40 transition-all flex flex-col justify-between">
                            <div className="space-y-2">
                                <div className="flex items-center justify-between text-amber-400 text-xs font-bold uppercase tracking-wider">
                                    <span className="flex items-center gap-1.5"><Zap className="w-4 h-4" /> Item Trap Warning</span>
                                    <span className="text-[10px] bg-amber-500/20 px-2 py-0.5 rounded text-amber-300 font-mono">Check Key</span>
                                </div>
                                <p className="text-sm font-bold text-slate-100">
                                    Question #{trickiestQuestion ? trickiestQuestion.qNum : 1} ({trickiestQuestion ? trickiestQuestion.topic : 'Mechanics'})
                                </p>
                                <p className="text-xs text-slate-300 leading-relaxed">
                                    High error rate ({trickiestQuestion ? trickiestQuestion.wrongPct : 52}% wrong). Check if distractor options created unintended confusion.
                                </p>
                            </div>
                            <div className="pt-2 border-t border-slate-700/60 text-[11px] text-amber-300 flex items-center justify-between">
                                <span>Discrim. Index: {trickiestQuestion ? trickiestQuestion.discriminationIndex : 0.45}</span>
                                <button className="font-semibold cursor-pointer hover:underline" onClick={() => setSelectedQuestionModal(trickiestQuestion)}>Audit Question →</button>
                            </div>
                        </div>

                        {/* Card 3: Learning Cohort Health */}
                        <div className="bg-slate-800/60 rounded-2xl p-4.5 border border-slate-700/60 space-y-2.5 hover:border-indigo-500/40 transition-all flex flex-col justify-between">
                            <div className="space-y-2">
                                <div className="flex items-center justify-between text-emerald-400 text-xs font-bold uppercase tracking-wider">
                                    <span className="flex items-center gap-1.5"><GraduationCap className="w-4 h-4" /> Learning Cohort Health</span>
                                    <span className="text-[10px] bg-emerald-500/20 px-2 py-0.5 rounded text-emerald-300 font-mono">Class Matrix</span>
                                </div>
                                <p className="text-sm font-bold text-slate-100">
                                    {cohortCounts.Mastery} in Mastery • {cohortCounts['At-Risk']} At-Risk
                                </p>
                                <p className="text-xs text-slate-300 leading-relaxed">
                                    Median score stands at <strong>{medianScore} / {testInfo.total_max_marks}</strong>. {cohortCounts.Impulsive} students rushed with negative marking penalty.
                                </p>
                            </div>
                            <div className="pt-2 border-t border-slate-700/60 text-[11px] text-emerald-300 flex items-center justify-between">
                                <span>Avg Completion: {completionRate}%</span>
                                <span className="font-semibold cursor-pointer hover:underline" onClick={() => setActiveTab('students')}>View Quadrants →</span>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="lg:col-span-2 bg-white rounded-3xl p-6 border border-slate-200/80 shadow-xs space-y-4">
                            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                                <div>
                                    ...
                                    <p className="text-xs text-slate-400">Student count broken down by score ranges</p>
                                </div>
                    ...
                    <div id="students-section" className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-8 border border-slate-200/80 shadow-xs space-y-4 sm:space-y-6">
                        ...
                        <div className="space-y-4">
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                <div>
                                    <h2 className="text-lg sm:text-xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
                                        ...
                                        Candidate Scorecards & Pedagogical Profiles
                                    </h2>
                            ...
                            <div className="flex flex-wrap items-center gap-2 pt-1 text-xs">
                                ...
                                >
                                    <option value="All">All Learning Archetypes</option>
                                    <option value="Mastery">🌟 Mastery Cohort</option>
                                    <option value="Deep Thinkers">⏳ Deep Thinkers</option>
                                    <option value="Impulsive">⚡ Impulsive Guessers</option>
                                    <option value="At-Risk">🚨 At-Risk Cohort</option>
                                </select>
                                ...
                                {(statusFilter !== 'All' || scoreRangeFilter !== 'All' || cohortFilter !== 'All' || searchQuery) && (
                                    <button
                                        onClick={() => {
                                            setStatusFilter('All');
                            ...
                            <table className="w-full min-w-[750px] text-left border-collapse text-xs">
                                <thead>
                                    <tr className="bg-slate-50/80 border-b border-slate-200/80 text-slate-500 font-semibold uppercase tracking-wider text-[11px] sticky top-0 backdrop-blur-md">
                                        ...
                                        <th className="p-3">Learning Profile</th>
                                        ...
                                        <th className="p-3 cursor-pointer hover:text-slate-900 transition-colors text-center" onClick={() => handleSort('percentage')}>
                                            Accuracy % {sortField === 'percentage' && (sortOrder === 'asc' ? '↑' : '↓')}
                                        </th>
                                ...
                                questions.map((q, idx) => {
                                    ...
                                    return (
                                        <div key={q.id || qNum || idx} className="bg-slate-50/70 rounded-2xl p-4 sm:p-5 border border-slate-200/70 hover:border-slate-300 transition-all flex flex-col justify-between space-y-4 group">
                                            ...
                                            {/* Distractor Frequency Bars (A, B, C, D) */}
                                            <div className="space-y-2 pt-2 border-t border-slate-200/60">
                                                ...
                                                <div className="grid grid-cols-4 gap-1.5 text-center text-[10px]">
                                                    {['A', 'B', 'C', 'D'].map((opt) => (
                                                        <div key={opt} className="bg-white p-1 rounded-md border border-slate-200">
                                                            <span className="text-slate-400 font-semibold">{opt}:</span>
                                                            <strong className="text-slate-800 ml-1">{(dist as any)[opt] || 0}%</strong>
                                                        </div>
                                                    ))}
                                                </div>
                                                <div className="flex justify-between text-[10px] text-slate-400 pt-1 font-medium">
                                                    ...
                                                    <span>Discrim: <strong className="text-slate-700">{discriminationIndex}</strong></span>
                                                </div>
                                            </div>
