                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                    ...
                                    <div className="rounded-xl border bg-card p-5 shadow-sm">
                                        ...
                                        {locationBarData.length > 0 ? (
                                            ...
                                            <div className="flex h-[300px] items-center justify-center text-muted-foreground">No location data yet.</div>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    <div className="relative max-w-sm">
                                        ...
                                        <input
                                            ...
                                            className="w-full rounded-lg border bg-card pl-9 pr-3 py-2 text-sm focus:outline-none"
                                        />
                                    </div>
                                        ...
                                        <table className="w-full text-sm">
                                            <thead>
                                                <tr className="border-b bg-muted/30">
                                                    <th onClick={() => toggleSort('title')} className="px-4 py-3 text-left font-semibold cursor-pointer">Title <SortIcon field="title" /></th>
                                                    <th onClick={() => toggleSort('creator_name')} className="px-4 py-3 text-left font-semibold cursor-pointer">Creator <SortIcon field="creator_name" /></th>
                                                    <th onClick={() => toggleSort('starts')} className="px-4 py-3 text-center font-semibold cursor-pointer">Starts <SortIcon field="starts" /></th>
                                                    <th onClick={() => toggleSort('submitted')} className="px-4 py-3 text-center font-semibold cursor-pointer">Submitted <SortIcon field="submitted" /></th>
                                                    <th onClick={() => toggleSort('abandoned')} className="px-4 py-3 text-center font-semibold cursor-pointer">Abandoned <SortIcon field="abandoned" /></th>
                                                    <th onClick={() => toggleSort('completion_rate')} className="px-4 py-3 text-center font-semibold cursor-pointer">Completion % <SortIcon field="completion_rate" /></th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y">
                                                {sorted.length === 0 ? (
                                                    <tr><td colSpan={6} className="px-4 py-12 text-center text-muted-foreground">No test attempts.</td></tr>
                                                ) : (
                                                    sorted.map((t: any, i: number) => (
                                                        <tr key={i} className="hover:bg-muted/20">
                                                            <td className="px-4 py-3 font-medium truncate max-w-[200px]">{t.title}</td>
                                                            <td className="px-4 py-3 text-muted-foreground">{t.creator_name}</td>
                                                            <td className="px-4 py-3 text-center font-mono">{t.starts}</td>
                                                            <td className="px-4 py-3 text-center text-green-600">{t.submitted}</td>
                                                            <td className="px-4 py-3 text-center text-red-600">{t.abandoned}</td>
                                                            <td className="px-4 py-3 text-center font-mono">{t.completion_rate}%</td>
                                                        </tr>
                                                    ))
                                                )}
                                            </tbody>
                                ...
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="border-b bg-muted/30">
                                            ...
                                            <th className="px-4 py-3 text-center font-semibold">Started</th>
                                    ...
                                    <tbody className="divide-y">
                                        {userMatrix.map((u: any, i: number) => (
                                            <tr key={i} className="hover:bg-muted/20">
                                                <td className="px-4 py-3 font-medium">{u.user_id === 'anonymous' ? 'Guests' : u.full_name}</td>
                                                <td className="px-4 py-3 text-muted-foreground text-xs">{u.email}</td>
                                                <td className="px-4 py-3 text-center font-mono">{u.tests_started}</td>
                                                <td className="px-4 py-3 text-center text-green-600">{u.submitted}</td>
                                                <td className="px-4 py-3 text-center font-mono">{u.avg_completion}%</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                        ...
                        {activeTab === 'creation' && (
                            <div className="space-y-6">
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                    <StatCard title="Tests Created" value={creationStats?.total_created || 0} icon={FileText} color="text-indigo-500" bgColor="bg-indigo-500/10" />
                                </div>
                                ...
                                <div className="rounded-xl border bg-card shadow-sm overflow-x-auto">
                                    ...
                                    <table className="w-full text-sm">
                                        ...
                                        <tbody className="divide-y">
                                            {(creationStats?.tests || []).slice(0, 10).map((t: any, i: number) => (
                                                <tr key={i} className="hover:bg-muted/20">
                                                    ...
                                                    <td className="px-4 py-3 text-center">
                                                        <span className="bg-slate-100 text-slate-800 text-xs px-2 py-0.5 rounded">{t.visibility || 'public'}</span>
                                                    </td>
                                                </tr>
                                            ))}
                                        </tbody>
                                ...
                                <div className="rounded-xl border bg-card shadow-sm overflow-x-auto">
                                    <div className="p-4 border-b">
                                        <h3 className="text-lg font-semibold">Recent Document Uploads</h3>
                                    </div>
                                    <table className="w-full text-sm">
                                        <thead>
                                            <tr className="border-b bg-muted/30">
                                        ...
                                        <tbody className="divide-y">
                                            {uploadLogs.length === 0 ? (
                                                <tr><td colSpan={3} className="px-4 py-12 text-center text-muted-foreground">No uploads found.</td></tr>
                                            ) : (
                                                uploadLogs.map((log: any, i: number) => (
                                                    <tr key={i} className="hover:bg-muted/20">
                                                        <td className="px-4 py-3 font-medium">{log.title}</td>
                                                        <td className="px-4 py-3 text-xs">{log.type}</td>
                                                        <td className="px-4 py-3 text-xs text-muted-foreground">{new Date(log.uploaded_at).toLocaleString()}</td>
                                                    </tr>
                                                ))
                                            )}
                                        </tbody>
                                    ...
                                    <tbody className="divide-y">
                                        {attemptLogs.map((log: any, i: number) => (
                                            <tr key={i} className="hover:bg-muted/20">
                                                ...
                                                <td className="px-4 py-3 truncate max-w-[200px]">{log.test_title}</td>
                                                <td className="px-4 py-3 text-center">{log.status}</td>
                                                <td className="px-4 py-3 text-center font-mono">{log.completion_pct}%</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
